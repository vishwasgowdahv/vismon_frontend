...
export default function UpdateWork(props) {
  const { setCloseAddWorkModal, workInfoFromProps, id } = props;
  const context = useContext(workContext);
  const { updateWork } = context;

  const [workInfo, setWorkInfo] = useState({
    companyName: workInfoFromProps.companyName,
    workStartDate: workInfoFromProps.workStartDate,
    workEndDate: workInfoFromProps.workEndDate,
    startTime: workInfoFromProps.startTime,
    endTime: workInfoFromProps.endTime,
    tookVehicleToGo: workInfoFromProps.tookVehicleToGo,
    tookVehicleToComeBack: workInfoFromProps.tookVehicleToComeBack,
    wagePerHour: workInfoFromProps.wagePerHour,
    breakTaken: workInfoFromProps.breakTaken,
  });

  //   let startWorkDate = workInfoFromProps.workStartDate.toString();
  //   startWorkDate = startWorkDate.slice(0, 10);
  //   console.log(startWorkDate);

  //   let endWorkDate = workInfoFromProps.workEndDate.toString();
  //   endWorkDate = endWorkDate.slice(0, 10);
  //   //   setWorkInfo({ ...workInfo, [workEndDate]: endWorkDate });
  ...
  const handleSubmit = () => {
    ...
    for (const key in workInfo) {
      if (key == breakTaken && workInfo[key] == "") {
        tempi = true;
        break;
      }
    }
